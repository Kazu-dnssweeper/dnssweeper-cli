...
describe('CLI E2E Tests', () => {
    ...
    const testDataFile = path_1.default.join(testDataDir, 'normal-records-50.csv');
    const tempDir = path_1.default.join(__dirname, '../temp');
    const tempFile = path_1.default.join(tempDir, 'test-e2e.csv');
    beforeAll(async () => {
...
test-server.example.com,A,192.168.1.20,300,false,2023-08-15T11:30:00Z,2023-08-15T11:30:00Z`;
        await fs_1.promises.writeFile(tempFile, testCSV);
    });
    describe('analyze command', () => {
        ...
        it('実際のテストデータファイルが分析できる', async () => {
            try {
                await fs_1.promises.access(testDataFile);
            }
            catch (error) {
    ...
    describe('output file functionality', () => {
        const outputFile = path_1.default.join(tempDir, 'test-output.csv');
        afterEach(async () => {
            try {
                await fs_1.promises.unlink(outputFile);
            }
            catch (error) {
            }
        });
        it('--output-file オプションが動作する', async () => {
            ...
            const fileExists = await fs_1.promises.access(outputFile).then(() => true).catch(() => false);
            expect(fileExists).toBe(true);
            const fileContent = await fs_1.promises.readFile(outputFile, 'utf-8');
        ...
        it('--risk-level と --output-file の組み合わせが動作する', async () => {
            ...
            const fileContent = await fs_1.promises.readFile(outputFile, 'utf-8');
            expect(fileContent).toContain('DNSweeper分析結果 - 月次DNS棚卸し用');
            expect(fileContent).toContain('old-api.example.com');
        }, 30000);
    ...
    describe('performance', () => {
        it('大量のレコードでも適切な時間で処理される', async () => {
            ...
            const largeTempFile = path_1.default.join(tempDir, 'large-test.csv');
            await fs_1.promises.writeFile(largeTempFile, largeCSV.join('\n'));
